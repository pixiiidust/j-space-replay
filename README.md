...
Watch what a local vision-language model reads internally while it answers
a question about a video.

Upload a short clip and ask a question. The tool replays the model's
decoded per-layer readouts on the video timeline. It runs one offline pass
over Qwen2.5-VL-7B (4-bit) on a single NVIDIA GPU.

Based on Anthropic's [workspace paper](https://transformer-circuits.pub/2026/workspace/index.html)
...
ball-drop clip (screenshot above). While the answer streams, the workspace
grid reads `gravity` at layers 22–25 several tokens before the model writes
it. The bands are visible: echo at early layers, workspace words in the
middle, the emitted token at layer 27.

**Adversarial checks.** When the answer rules something out ("there is no
visible evidence of the panda being pushed"), cells whose readouts still
contain the ruled-out word (`pushed`) pulse with a red outline. The
workspace reads what the reply rejects.

**Unspoken readouts.** A strip above the grid lists words the model reads
across many cells that appear in neither the prompt nor the answer. Click
one to highlight its cells. This is the paper's suppression signal:
content readable internally that the output never says.

**Computed but unsaid.** On the ball-drop clip, layer ~24 patch readouts
move `level → horizontal, move → off, right` as the ball rolls and falls.
The answer (a generic gravity explanation) never mentions any of this.

These are diagnostic signals, not mind-reading. See the honesty section.

## Status

Working demo. Every trace is badged with the lens that decoded it; the two
lenses cache separately and never overwrite each other.
...
cell, click to drill to raw top-10), adversarial pulses, the unspoken
strip, late-layer patch readouts, instant replay of cached traces.

**What's demo-quality:** the adversarial/unspoken extraction (mechanical
string analysis of decoded tokens), concept labels (kept in the trace,
no longer a UI panel), patch overlays, synthetic-clip baselines.
...
- **Query console** (top): prompt in, answer out, token by token. Click an
  answer token to seek the replay. Lens picker next to re-ask. An
  adversarial line appears when the answer rules something out.
- **Workspace grid** (hero): answer-token × layer, top-1 word printed in
  ...
  as the clip plays. The unspoken-readouts strip sits above it.
...
checkpoint weights. Every component is verified against autograd; errors
and the method's stated approximations are in `reports/jlens_evidence.md`.
...
- Single-token readouts only; multi-token concepts need the paper's
  extension.
- The adversarial and unspoken surfaces are mechanical string analysis of
  decoded tokens. Unusual phrasings produce no flag rather than a wrong
  one.
